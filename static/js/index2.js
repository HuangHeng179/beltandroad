!(function(global) {
    ...
    let colors = ['rgba(51, 152, 225, 1)', 'rgba(92, 176, 233, 1)', 'rgba(134, 190, 231, 1)'];
    let mapArea = {
        color: {
            getColorValue(index) {
                return colors[index]
    ...
    function error(msg) {
        if (console && console.log && typeof console.error === 'function') {
            console.error(msg);
        }
    }
    ...
    fetch({
        ...
        echarts.registerMap('WorldMap', res.data);
        let option = {
            backgroundColor: '#020c5e',
            ...
            series: [
                //根据经纬度在地图上描点
                {
                    ...
                    data: warMapData.map(function (itemOpt) {
                        return {
                            name: itemOpt.name,
                            value: [
                                itemOpt.longitude,
